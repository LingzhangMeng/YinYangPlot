...
counts <- read.table(
   "//path...//gene_counts.txt",   # Locate your files on your computer
   header = TRUE,
   row.names = 1,
   comment.char = "#",
   check.names = FALSE,
...
Before running DESeq2, we define the experimental design by creating a metadata table (`colData`) that assigns each sample to a condition.
...
We initialize the `DESeqDataSet` object and perform a pre-filtering step to remove genes with low counts across all samples, which improves statistical power.
...
**Would you like me to continue with the next sections, such as Normalization, PCA plots, or the actual Differential Expression results?**

## Response:
Continuing with the workflow, this section covers the core statistical analysis using DESeq2, Log-Fold Change (LFC) shrinkage, and a robust multi-method annotation strategy to map Ensembl IDs to gene symbols and biotypes.
...
We execute the DESeq2 pipeline, which includes size factor estimation, dispersion estimation, and model fitting. To improve visualization and ranking of genes, we apply the `apeglm` shrinkage method to the  $log_{2}$  Fold Changes.
...
We categorize genes into functional groups (Protein-coding, lncRNA, miRNA, etc.) and define significance thresholds ( $p_{adj}<0.05$  and  $∣log_{2}FC∣>0.3$ ) for downstream visualization like Volcano or Yin-Yang plots.
...
**Would you like me to help you format the final section for creating the Yin-Yang plots or the interactive Plotly visualizations?**

## Response:
In this final section, we refine the statistical significance flags and implement an automated file management system to export the results. The workflow concludes by segregating the large dataset into specific files based on gene biotype, facilitating targeted downstream analysis for protein-coding genes, miRNAs, and lncRNAs.
...
To identify the most biological relevant changes, we apply a stricter significance threshold ( $P<0.05$  and  $∣log_{2}FC∣>1$ ) and sort the data by significance.
...
output_dir <- "F:/Chunxia/r_analysis/4. Processing/Yinyang_CSV files"
...
**Would you like me to help you generate a visual summary like a Volcano Plot or a Heatmap for these top genes?**
...
To prioritize genes with the strongest evidence of differential expression, we apply a more localized filtering strategy. This step separates genes based on both statistical significance ( $p_{adj}<0.05$ ) and biological magnitude ( $∣log_{2}FC∣>0.3$ ).
...
**Would you like me to help you draft the "Usage" or "Installation" section for this GitHub repository now that the scripts are complete?**
...
After segregating the data, we focus on the **protein-coding** subset to generate publication-quality figures. We utilize two primary visualization methods: a customized Volcano Plot and the unique Yin-Yang Plot.
...
Upon completion, all statistical tables and summaries are saved to: `F:/Chunxia/r_analysis/4. Processing/Yinyang_CSV files/`
...
**This completes the RNA-Seq README documentation. Would you like me to add a "Troubleshooting" section regarding the biomaRt connection or the EnsDb installation?**
...
**Would you like me to compile all these sections into a single `README.md` file code block that you can copy and paste directly into GitHub?**
